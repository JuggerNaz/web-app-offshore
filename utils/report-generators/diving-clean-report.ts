...
export const generateDivingCLEANReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF, pageNo: number) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Cleaning Inspection Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        const drawPageFooter = (d: jsPDF, pageNo: number) => {
            ...
            d.text(
                `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Cleaning Inspection Report (Diving)  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                margin, pageHeight - 6
            );
        ...
        doc.save(`Diving_CLEAN_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
