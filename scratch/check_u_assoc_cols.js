...
async function run() {
  ...
  try {
    ...
    const result = await conn.execute(
      `SELECT COLUMN_NAME, DATA_TYPE FROM USER_TAB_COLUMNS WHERE TABLE_NAME = 'U_ASSOC'`
    );
    
    console.log("\n--- U_ASSOC columns ---");
    result.rows.forEach(r => console.log(`${r[0]}: ${r[1]}`));

    console.log("\n--- Sample row from U_ASSOC ---");
    const sample = await conn.execute(
      `SELECT * FROM U_ASSOC WHERE ROWNUM = 1`
    );
    if (sample.rows && sample.rows.length > 0) {
      ...
      console.log("No rows in U_ASSOC");
    }
