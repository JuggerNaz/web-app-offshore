...
export const GET = withAuth(
  async (
    ...
    // 2. Try fetching from webapp_3d database table first
    const { data: webapp3d, error } = await (adminSupabase as any)
      .from("webapp_3d")
      .select("*")
    ...
    const rawCompMap = new Map<number, any>();
    (rawComponents || []).forEach((c: any) => rawCompMap.set(Number(c.id), c));

    const excludeCodes = ["IT", "FV", "HS", "GP", "PG", "PC", "RC", "RB", "SD", "FA"];
    const webapp3dMap = new Map<number, any>();
    if (webapp3d) {
      webapp3d.forEach((w: any) => {
        const cid = Number(w.component_id || w.custom_props?.comp_id || w.comp_id);
        if (cid && !isNaN(cid)) webapp3dMap.set(cid, w);
      });
    }
    ...
    if (webapp3d && webapp3d.length > 0 && !forceResync) {
      const unlinkedCount = webapp3d.filter((w: any) => {
        const cid = Number(w.component_id || w.custom_props?.comp_id || w.comp_id);
        return !cid || !rawCompMap.has(cid);
      }).length;
      const isUnlinked = unlinkedCount > webapp3d.length * 0.3; // if > 30% unlinked, force auto-heal resync

      const legCoords = webapp3d
        .filter((w: any) => {
          const code = (w.custom_props?.code || w.code || "").toUpperCase();
          return code.includes("LG") || code === "LEG";
        })
    ...
    if (webapp3d && webapp3d.length > 0 && !isDegenerate) {
      ...
      componentsToEnrich = webapp3d.filter((item: any) => {
        const cid = Number(item.component_id || item.custom_props?.comp_id || item.comp_id);
        const sc = rawCompMap.get(cid);
        return !sc || !sc.is_deleted;
      });
      ...
      componentsToEnrich = componentsToEnrich.map((item: any) => {
        const cid = Number(item.component_id || item.custom_props?.comp_id || item.comp_id);
        const sc = rawCompMap.get(cid) || item.structure_components || {};
        const realQId = sc.q_id || item.custom_props?.q_id || (item.q_id && !item.q_id.startsWith("COMP-") ? item.q_id : null) || sc.name || item.q_id;
        const qid = String(realQId || "").toUpperCase().trim();
        const mathLayout = mathLayoutsByCompId.get(cid) || mathLayoutsByQId.get(qid);

        const code = (sc.code || item.custom_props?.code || item.code || "").toUpperCase();
        const isPile = code === "PL" || code === "PILE" || qid.includes("PILE");

        let updatedItem = {
          ...item,
          ...sc,
          id: sc.id || cid || item.id,
          comp_id: sc.id || cid || item.comp_id,
          component_id: item.component_id || String(sc.id || cid),
          q_id: realQId,
          code: code || item.code,
          structure_components: sc,
          thickness: item.dimensions?.thickness || item.thickness || 0.2,
          color_hex: item.color_hex || sc.color || "#64748b",
        };
        ...
        if ((isZeroOrPoint || isSupportWeld || isCaisson) && mathLayout) {
          ...
          updatedItem = {
            ...
            pos_y: (sY + eY) / 2,
            pos_z: (sZ + eZ) / 2,
            thickness: mathLayout.thickness || item.dimensions?.thickness || item.thickness || 0.2,
          };
          return updatedItem;
        }
        return updatedItem;
      ...
      const cachedCompIds = new Set(componentsToEnrich.map((w: any) => Number(w.component_id || w.comp_id || w.id)).filter(Boolean));
      const cachedQIds = new Set(componentsToEnrich.map((w: any) => (w.q_id || "").toUpperCase()).filter(Boolean));
      ...
      if (isDegenerate) {
        await (adminSupabase as any).from("webapp_3d").delete().eq("structure_id", structureIdNum);
      }
      ...
      // Auto-populate webapp_3d table in Postgres synchronously
      if (componentsToEnrich.length > 0) {
        try {
          const insertData = componentsToEnrich.map((item: any) => ({
            structure_id: structureIdNum,
            component_id: String(item.component_id || item.comp_id),
            pos_x: item.pos_x,
            pos_y: item.pos_y,
            pos_z: item.pos_z,
            rot_x: item.rot_x || 0,
            ...
            scale_z: item.scale_z || 1,
            shape_type: item.shape_type || "cylinder",
            dimensions: item.dimensions || { length: item.length, radius: item.thickness || 0.3, thickness: item.thickness || 0.3 },
          ...
          for (let i = 0; i < insertData.length; i += chunkSize) {
            ...
            const { error: insErr } = await (adminSupabase as any).from("webapp_3d").insert(chunk);
            if (insErr) {
              console.error("[webapp-3d] Insert chunk error:", insErr);
            }
          }
        } catch (syncErr) {
          console.error("[webapp-3d] Auto-population sync failed:", syncErr);
        }
      }
    }
