...
async function run() {
    try {
        ...
        // PASS 1: Authoritative Root Nodes (WN)
        components.forEach((c) => {
            const md = c.metadata || {};
            const code = (c.code || "").toUpperCase();
        ...
        // PASS 1.2: Register endpoints for primary member components
        components.forEach((c) => {
            const md = c.metadata || {};
            const code = (c.code || "").toUpperCase();
        ...
        // PASS 1.7: child node welds (WN) on parent members
        ...
        console.log("Interpolating child welds...");
        for (let iter = 0; iter < 3; iter++) {
            childWeldGroups.forEach((children, parentId) => {
                ...
                const startNodeWeld = components.find(w => {
                    const code = (w.code || "").toUpperCase();
                    const isWeld = code === "WN" || code === "WP" || code.includes("WELD");
                    return isWeld && (w.metadata?.s_node || "").toUpperCase().trim() === (parentMd.s_node || "").toUpperCase().trim();
                });
                ...
                const startDist = startNodeWeld ? parseFloat(startNodeWeld.metadata?.dist || "0") : 0;
                const endDist = endNodeWeld ? parseFloat(endNodeWeld.metadata?.dist || "0") : 0;
                ...
                children.forEach((c, idx) => {
                    ...
                    if (!isNaN(distVal) && distVal > 0) {
                        ...
                        if (endDist > startDist + 0.01) {
                            const relativeDist = distVal - startDist;
                            const relativeLength = endDist - startDist;
                            t = Math.max(0, Math.min(1, relativeDist / relativeLength));
                        } else if (endDist > 0.01) {
                            t = Math.max(0, Math.min(1, distVal / endDist));
        ...
        // Now evaluate conductor logic (ELEVATION BY ELEVATION MULTI-SEGMENT RESOLUTION)
        console.log("\nResolving Conductors multi-segment:");
        const conductors = components.filter(c => c.code === "CD" || (c.q_id || "").toUpperCase().includes("CON"));
        conductors.forEach(c => {
            const md = c.metadata || {};
            const topElv = sanitizeElevation(md.elv_1 || 3);
            const botElv = sanitizeElevation(md.elv_2 || -55);
            
            // Extract the numeric index of the conductor (e.g. "CON001" -> "001")
            const condName = (c.q_id || "").toUpperCase();
            const numMatch = condName.match(/\d+/);
            if (!numMatch) {
                console.log(`Conductor ${c.q_id} has no numeric index!`);
                return;
            }
            const indexStr = numMatch[0]; // e.g. "001"
            ...
            matchingCgfs.forEach(cgf => {
                ...
                if (sPos && fPos) {
                    ...
                    cgfMidpoints.push({ elv, midpoint, qId: cgf.q_id });
                }
            });
            ...
            console.log(`Conductor ${c.q_id} found ${cgfMidpoints.length} guide frames:`);
            cgfMidpoints.forEach(m => {
                console.log(`  Frame: ${m.qId} at Elv ${m.elv}m -> Midpoint: [${m.midpoint.x.toFixed(3)}, ${m.midpoint.y.toFixed(3)}, ${m.midpoint.z.toFixed(3)}]`);
            });

            // Generate segments
            if (cgfMidpoints.length > 0) {
                // If there's only one midpoint (or multiple), we can generate segments between consecutive ones.
                for (let i = 0; i < cgfMidpoints.length - 1; i++) {
                    const startMid = cgfMidpoints[i];
                    const endMid = cgfMidpoints[i+1];
                    console.log(`  Segment ${i+1}: from [${startMid.midpoint.x.toFixed(3)}, ${startMid.elv.toFixed(3)}, ${startMid.midpoint.z.toFixed(3)}] to [${endMid.midpoint.x.toFixed(3)}, ${endMid.elv.toFixed(3)}, ${endMid.midpoint.z.toFixed(3)}]`);
                }
                // Finally, project the last guide frame down to the bottom elevation botElv if it is deeper than the last frame
                const lastFrame = cgfMidpoints[cgfMidpoints.length - 1];
                if (botElv < lastFrame.elv - 0.1) {
                    console.log(`  Bottom Extension Segment: from [${lastFrame.midpoint.x.toFixed(3)}, ${lastFrame.elv.toFixed(3)}, ${lastFrame.midpoint.z.toFixed(3)}] to [${lastFrame.midpoint.x.toFixed(3)}, ${botElv.toFixed(3)}, ${lastFrame.midpoint.z.toFixed(3)}]`);
                }
                // Also, project the first guide frame up to the top elevation topElv if it is higher than the first frame
                const firstFrame = cgfMidpoints[0];
                if (topElv > firstFrame.elv + 0.1) {
                    console.log(`  Top Extension Segment: from [${firstFrame.midpoint.x.toFixed(3)}, ${topElv.toFixed(3)}, ${firstFrame.midpoint.z.toFixed(3)}] to [${firstFrame.midpoint.x.toFixed(3)}, ${firstFrame.elv.toFixed(3)}, ${firstFrame.midpoint.z.toFixed(3)}]`);
                }
            } else {
                console.log(`  No guide frames matched! Fallback to s_node.`);
            }
        });
