...
/**
 * ROV Seabed Survey Inspection Report (Portrait)
 * Columns: Item No. | QID | Dive No. | Tape No. | Findings
 *
 * Ordered by elevation and QID.
 */
export const generateROVRSEABDetailReport = async (
    ...
    try {
        ...
        // ── Filter Records (Strict Seabed Filter: RSEAB only) ──
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.text("Seabed Survey Inspection Report (ROV)", margin + contentWidth / 2, margin + 17, { align: "center" });
        ...
        doc.save(`ROV_Seabed_Survey_Inspection_Report_${headerData.sowReportNo}_${format(new Date(), 'yyyyMMdd')}.pdf`);
    } catch (e) {
