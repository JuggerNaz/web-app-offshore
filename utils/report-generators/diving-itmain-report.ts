...
/**
 * Item Maintenance Inspection Report (Diving) — Portrait Mode
 * Specific for Inspection Type ITMAIN
 * 
 * Columns: Item No. | QID | Elevation (m) | Dive No. | Angle (°) | Dim 1 (m) | Dim 2 (m) | Dim 3 (m) | Findings
 */
export const generateDivingITMAINReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Item Maintenance Inspection Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        const buildRow = (r: any, idx: number): string[] => {
            ...
            // Angle formatting with unit
            const rawAngle = d.angle ?? r.angle;
            const angleDisplay = (rawAngle !== undefined && rawAngle !== null && String(rawAngle).trim() !== "")
                ? (String(rawAngle).includes("°") || String(rawAngle).toLowerCase().includes("deg") ? String(rawAngle) : `${rawAngle}°`)
                : "—";

            // Dimension 1 formatting
            const rawDim1 = d.dimension1 ?? d.dim1 ?? r.dimension1;
            const dim1Display = (rawDim1 !== undefined && rawDim1 !== null && String(rawDim1).trim() !== "")
                ? (String(rawDim1).toLowerCase().includes("m") || String(rawDim1).toLowerCase().includes("mm") ? String(rawDim1) : `${rawDim1} m`)
                : "—";

            // Dimension 2 formatting
            const rawDim2 = d.dimension2 ?? d.dim2 ?? r.dimension2;
            const dim2Display = (rawDim2 !== undefined && rawDim2 !== null && String(rawDim2).trim() !== "")
                ? (String(rawDim2).toLowerCase().includes("m") || String(rawDim2).toLowerCase().includes("mm") ? String(rawDim2) : `${rawDim2} m`)
                : "—";

            // Dimension 3 formatting
            const rawDim3 = d.dimension3 ?? d.dim3 ?? r.dimension3;
            const dim3Display = (rawDim3 !== undefined && rawDim3 !== null && String(rawDim3).trim() !== "")
                ? (String(rawDim3).toLowerCase().includes("m") || String(rawDim3).toLowerCase().includes("mm") ? String(rawDim3) : `${rawDim3} m`)
                : "—";

            // Findings column & Postfix handling
            const findingsParts: string[] = [];

            // 1) Main Remarks / Findings / Comments text
            const mainFinding = 
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                ...
                5: { cellWidth: 16, halign: "center" },   // Dim 1
                6: { cellWidth: 16, halign: "center" },   // Dim 2
                7: { cellWidth: 16, halign: "center" },   // Dim 3
                8: { cellWidth: "auto" },                 // Findings
            },
            didParseCell: (data) => {
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`Diving_Item_Maintenance_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[Item Maintenance Inspection (Diving) Report] Error:", err);
        throw err;
    }
};
