...
/**
 * ROV Water Depth Inspection Report (RWDI) — Portrait
 *
 * Columns: Item No. | QID | Elevation (m) | Dive No. | Water Depth (m) | Findings
 */
export const generateROVRWDIReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.text("ROV Water Depth Inspection Report",                                 margin + contentWidth / 2, margin + 17, { align: "center" });
        ...
        autoTable(doc, {
            ...
            didDrawPage: (data) => {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  ROV Water Depth Inspection Report  |  SOW: ${headerData.sowReportNo || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        if (config.showSignatures !== false) {
            ...
            // If not enough space for signature, add new page
        ...
        doc.save(`ROV_Water_Depth_Report_${headerData.sowReportNo || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
