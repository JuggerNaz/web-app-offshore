...
export const generateROVCondReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11);  d.setFont("helvetica", "bold");
            d.text("Conductor Survey Report (ROV)",                                margin + contentWidth / 2, margin + 16.5, { align: "center" });
        ...
        sortedParentIds.forEach((parentId, groupIdx) => {
            ...
            autoTable(doc, {
                ...
                didDrawPage: (data) => {
                    ...
                    doc.text(
                        `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Conductor Survey Report (ROV)  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                        margin, pageHeight - 6
                    );
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`ROV_Conductor_Survey_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
