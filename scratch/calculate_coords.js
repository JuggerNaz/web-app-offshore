...
// Simple vector implementation similar to THREE.Vector3
class Vec3 {
    ...
    try {
        ...
        for (let iter = 0; iter < 3; iter++) {
            childWeldGroups.forEach((children, parentId) => {
                ...
                if (!pStart || !pEnd) {
                    if (iter === 0) console.log(`Parent ${parentId} missing endpoints: start=${parentMd.s_node} (${!!pStart}), end=${parentMd.f_node} (${!!pEnd})`);
                    return;
                }
