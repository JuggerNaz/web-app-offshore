...
export function FacesDialog() {
    ...
    const legs = Object.keys(data.data).filter(x => x.startsWith('leg_t')).map(x => {
                ...
                <div className="grid items-center gap-4">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            <RowWrap>
                                <ColWrap>
                                    <FormFieldWrap label="Name" name="face" form={form} placeholder="" ftype="normal" type="text" />
                                    <FormFieldWrap label="Description" name="description" form={form} placeholder="" ftype="normal" type="text" />
                                    <FormFieldWrap label="From" name="face_from" options={legs} form={form} ftype="select" />
                                    <FormFieldWrap label="To" name="face_to" options={legs} form={form} ftype="select" />
                                    <FormFieldWrap label="Work Unit" name="workunit" form={form} placeholder="" ftype="normal" maxLength="3" />
                                </ColWrap>
                            </RowWrap>
