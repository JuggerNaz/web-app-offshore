...
export function ChangelogCard({ entry }: { entry: ChangelogEntry }) {
  ...
  const customComponents: Components = {
    ...
    h4: ({ children, ...props }) => (
      <h5 className="text-sm font-semibold text-foreground mb-2 mt-2" {...props}>
        {children}
      </h5>
    ),
    p: ({ children, ...props }) => (
    ...
    ul: ({ children, ...props }) => (
      <ul className="list-disc list-outside space-y-1 mb-3 ml-6" {...props}>
        {children}
      </ul>
    ),
    ol: ({ children, ...props }) => (
      <ol className="list-decimal list-outside space-y-1 mb-3 ml-6" {...props}>
        {children}
      </ol>
    ),
    li: ({ children, ...props }) => (
    ...
    code: ({ children, ...props }) => {
      ...
      return (
        <code
          className="block bg-muted p-3 rounded-md overflow-x-auto text-xs font-mono mb-3"
          {...props}
        >
          {children}
        </code>
    ...
    pre: ({ children, ...props }) => (
      <pre className="bg-muted p-3 rounded-md overflow-x-auto text-xs font-mono mb-3" {...props}>
        {children}
      </pre>
    ),
    blockquote: ({ children, ...props }) => (
      <blockquote
        className="border-l-4 border-primary pl-4 italic mb-3 text-muted-foreground"
        {...props}
      >
        {children}
      </blockquote>
    ...
    table: ({ children, ...props }) => (
      <table className="w-full border-collapse border border-border mb-3" {...props}>
        {children}
      </table>
    ),
    ...
    th: ({ children, ...props }) => (
      <th className="border border-border px-2 py-1 text-sm font-semibold" {...props}>
        {children}
      </th>
    ),
  };
    ...
    <Card className="w-full">
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-2 flex-1">
            ...
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="text-sm">{formatChangelogDate(entry.metadata.date)}</span>
            </div>
          </div>
          <button
      ...
      <CardContent>
        <div className="space-y-4">
          {!isExpanded && <p className="text-muted-foreground">{getPreview(entry.content)}</p>}

          {isExpanded && (
            <div className="prose prose-sm max-w-none text-muted-foreground">
              <ReactMarkdown components={customComponents}>{entry.content}</ReactMarkdown>
            </div>
          )}
        </div>
      </CardContent>
