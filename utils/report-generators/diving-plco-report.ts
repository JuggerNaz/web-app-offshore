...
/**
 * Diving Coating Damage Inspection Report (PL_CO) — Landscape
 *
 * Columns: Item No. | QID | Elevation (m) | Dive No. | Surface Condition | CP Reading (mV) | Length (mm) | Width (mm) | Assessment | Findings
 */
export const generateDivingPLCOReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.text("Diving Coating Damage Inspection Report",                            margin + contentWidth / 2, margin + 17, { align: "center" });
        ...
        const buildRow = (r: any, idx: number): string[] => {
            ...
            const cpReading = d.cp_rdg !== undefined && d.cp_rdg !== null ? d.cp_rdg : (d.cp_reading_mv !== undefined && d.cp_reading_mv !== null ? d.cp_reading_mv : "—");
            const length = d.length !== undefined && d.length !== null ? d.length : "—";
            const width = d.width !== undefined && d.width !== null ? d.width : "—";
        ...
        autoTable(doc, {
            ...
            head: [[
                ...
                { content: "Length\n(mm)",          styles: { halign: "center", valign: "middle" } },
                { content: "Width\n(mm)",           styles: { halign: "center", valign: "middle" } },
            ...
            didDrawPage: (data) => {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Diving Coating Damage Inspection Report  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`Diving_Coating_Damage_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[Diving Coating Damage Report] Error:", err);
        throw err;
    }
};
