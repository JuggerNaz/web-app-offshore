...
export const generateDivingUTCLBReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("UT Calibration Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        autoTable(doc, {
            ...
            didDrawPage: (data) => {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  UT Calibration Report (Diving)  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
