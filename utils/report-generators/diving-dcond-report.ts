...
/**
 * Conductor Inspection Report (Diving)
...
export const generateDivingDCONDReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Conductor Inspection Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        doc.save(`Diving_Conductor_Inspection_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[generateDivingDCONDReport] Error:", err);
        throw err;
    }
};
