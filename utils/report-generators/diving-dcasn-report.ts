...
/**
 * Diving Caisson Inspection Report (Portrait)
 * Grouped by parent Caisson (CS) component.
 * Retrieves all caisson inspected records (both +ve and -ve elevations).
 * Compiles GVINS, CVINS, CPSURV, UTWTK.
 */
export const generateDivingDCASNReport = async (
    records: any[],
    headerData: any,
    ...
    console.log("[Diving Caisson Report] Starting generation", { recordsCount: records?.length, config });
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Caisson Inspection Diving", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        sortedCaissonQids.forEach((caissonQid, groupIdx) => {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Caisson Inspection Diving  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        console.log("[Diving Caisson Report] Generation complete, returnBlob:", config?.returnBlob);
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`Caisson_Inspection_Diving_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[Diving Caisson Report] Error:", err);
        throw err;
    }
};
