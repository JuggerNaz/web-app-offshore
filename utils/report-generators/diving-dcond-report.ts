...
/**
 * Conductor Inspection Diving Report (Portrait)
 * Grouped by parent Conductor (CD) component.
 * Retrieves all conductor inspected records (both +ve and -ve elevations).
 * Compiles GVINS, CVINS, CPSURV, UTWTK.
 */
export const generateDivingDCONDReport = async (
    records: any[],
    headerData: any,
    ...
    console.log("[Diving Conductor Report] Starting generation", { recordsCount: records?.length, config });
    ...
    try {
        ...
        // Filter records: type in [GVINS, CVINS, CPSURV, UTWTK, DUTWT] (ALL ELEVATIONS +ve and -ve)
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Conductor Inspection Diving", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        // Grouping logic (same as underwater conductor report)
        ...
        const getGroupKey = (r: any): string => {
            ...
            const findUltimateCDParent = (cid: number | null, depth = 0): string | null => {
                ...
                if (["CD", "CON"].includes(typeCode) && !pId) return c.q_id;
                return findUltimateCDParent(pId, depth + 1) || (["CD", "CON"].includes(typeCode) ? c.q_id : null);
            };
            ...
            if (qid.startsWith("CD") || qid.startsWith("CON")) {
                ...
                allComps?.forEach(c => {
                    ...
                    if (["CD", "CON"].includes(cCode) && !cpId && qid.startsWith(cQid) && cQid.length > bestMatch.length) {
                        bestMatch = c.q_id;
                    }
                });
            ...
            const match = qid.match(/^(CD-[^-_ ]+|CON-[^-_ ]+)/i);
            if (match) return match[1];

            return ["CD", "CON"].includes((comp.code || "").toUpperCase()) ? qid : "General";
        };
        ...
        sortedConductorQids.forEach((conductorQid, groupIdx) => {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Conductor Inspection Diving  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        console.log("[Diving Conductor Report] Generation complete, returnBlob:", config?.returnBlob);
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`Conductor_Inspection_Diving_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[Diving Conductor Report] Error:", err);
        throw err;
    }
};
