...
async function run() {
    try {
        ...
        let components = rawComps.filter((c) => {
        ...
        // We re-associate WN N13 (ID 131466) and WN N19 (ID 131467) to HOM N1-N2 (ID 130816) instead of their current parent members.
        components = components.map(c => {
            if (c.id === 131466) { // WN N13
                return {
                    ...c,
                    metadata: {
                        ...c.metadata,
                        associated_comp_id: 130816, // HOM N1-N2
                        dist: "4.19",
                        f_leg: "B2",
                        s_leg: "B1" // runs along HOM N1-N2 between B2 and B1
                    }
                };
            }
            if (c.id === 131467) { // WN N19
                return {
                    ...c,
                    metadata: {
                        ...c.metadata,
        ...
        console.log("Interpolating child welds (WITH DATABASE FIX):");
            ...
            childWeldGroups.forEach((children, parentId) => {
                ...
                children.forEach((c, idx) => {
                    ...
                    if (["WN N12", "WN N18", "WN N13", "WN N19"].includes(c.metadata?.description)) {
                        ...
                        console.log(`  dist=${c.metadata.dist}, realParentLength=${realParentLength}, t=${t.toFixed(4)}`);
