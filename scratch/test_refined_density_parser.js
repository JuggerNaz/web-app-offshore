const densityRegex = new RegExp("density(?:\\s*value)?(?:\\s*:|\\s)?\\s*([0-9]+(?:\\.[0-9]+)?)\\s*(?:g/cm3|g/cc|kg/m3)?", "i");
...
testComments.forEach(tc => {
  ...
  if (match) {
    ...
    let cutText = tc.replace(match[0], "");
    // Clean up multiple semicolons, leading/trailing semicolons, and spaces
    cutText = cutText.replace(/;{2,}/g, ';')
                     .replace(/^\s*;\s*/, '')
                     .replace(/\s*;\s*$/, '')
                     .trim();
