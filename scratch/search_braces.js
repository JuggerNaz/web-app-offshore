...
async function run() {
    try {
        ...
        const matches = comps.filter(c => {
            const md = c.metadata || {};
            const s = (md.s_node || "").toUpperCase().trim();
            const f = (md.f_node || "").toUpperCase().trim();
            return (s === "18" && f === "12") || (s === "12" && f === "18") ||
                   (s === "19" && f === "13") || (s === "13" && f === "19") ||
                   (s === "WN N18" && f === "WN N12") || (s === "WN N12" && f === "WN N18");
        });

        console.log(`Found ${matches.length} matching braces:`);
        matches.forEach(c => {
