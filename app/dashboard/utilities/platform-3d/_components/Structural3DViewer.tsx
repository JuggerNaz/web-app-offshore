...
function InstancedComponentViewer({
    ...
    useLayoutEffect(() => {
        ...
        cylinders.forEach((item, i) => {
            ...
            const modeColor = getComponentColorByMode(item.comp, isSelected, colorMode, currentRecords, historicalRecords, selectedCampaignId, baseColor, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent);
            const finalColor = colorMode && colorMode !== "DEFAULT" ? modeColor : getInspectionColor(item, modeColor);
    ...
    }, [cylinders, selectedCompId, mainMemberIds, colorMode, currentRecords, historicalRecords, selectedCampaignId, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent, isInspectionMode, selectedInspectionFilters]);
    ...
    useLayoutEffect(() => {
        ...
        welds.forEach((item, i) => {
            ...
            const compId = item.comp?.id || item.id;
            const isSelected = selectedCompId === compId;
            const baseColor = isSelected ? "#f97316" : "#94a3b8";
            const modeColor = getComponentColorByMode(item.comp, isSelected, colorMode, currentRecords, historicalRecords, selectedCampaignId, baseColor, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent);
            const finalColor = colorMode && colorMode !== "DEFAULT" ? modeColor : getInspectionColor(item, modeColor);
            color.set(finalColor);
            mesh.setColorAt(i, color);
        ...
        mesh.instanceMatrix.needsUpdate = true;
        if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
    }, [welds, selectedCompId, mainNodeIds, colorMode, currentRecords, historicalRecords, selectedCampaignId, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent, isInspectionMode, selectedInspectionFilters]);
    ...
    useLayoutEffect(() => {
        ...
        mesh.instanceMatrix.needsUpdate = true;
        if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
    }, [spheres, selectedCompId, mainNodeIds, colorMode, currentRecords, historicalRecords, selectedCampaignId, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent, isInspectionMode, selectedInspectionFilters]);
    ...
    useLayoutEffect(() => {
        ...
        boxes.forEach((item, i) => {
            ...
            const compId = item.comp?.id || item.id;
            const isSelected = selectedCompId === compId;
            const baseColor = isSelected ? "#f97316" : "#d97706";
            const modeColor = getComponentColorByMode(item.comp, isSelected, colorMode, currentRecords, historicalRecords, selectedCampaignId, baseColor, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent);
            const finalColor = colorMode && colorMode !== "DEFAULT" ? modeColor : getInspectionColor(item, modeColor);
            color.set(finalColor);
            mesh.setColorAt(i, color);
        ...
        mesh.instanceMatrix.needsUpdate = true;
        if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
    }, [boxes, selectedCompId, colorMode, currentRecords, historicalRecords, selectedCampaignId, libraryColors, selectedPriorityFilter, inspectionModeFilter, priorityScope, compareWithCurrent, isInspectionMode, selectedInspectionFilters]);
...
export function Structural3DViewer({
    ...
    const [isInspectionMode, setIsInspectionMode] = useState(externalIsInspectionMode);
    const [selectedInspectionFilters, setSelectedInspectionFilters] = useState<string[]>(externalSelectedInspectionFilters);

    // Keep local inspection state in sync when a parent controls it via props (page-driven inspection flow)
    useEffect(() => {
        setIsInspectionMode(externalIsInspectionMode);
    }, [externalIsInspectionMode]);
    useEffect(() => {
        ...
        const layouts = (webapp3dData.components || []).map((dbItem: any) => {
            ...
            if (!comp) {
                comp = rawComponents.find((c: any) => {
                    ...
                    if (baseCompIdStr && cIdStr && cIdStr === baseCompIdStr) return true;
            ...
            const q_id = comp.q_id || dbItem.q_id || dbItem.structure_components?.q_id || `COMP-${baseCompIdStr || dbItem.component_id}`;
            ...
            return {
                ...
                color: finalColor,
                thickness: clampThickness || ((isPile && !isPileLegComponent) ? 0.08 : (dbItem.thickness || dbItem.dimensions?.radius || (isPileLegComponent ? 2.5 : defaultThickness))),
