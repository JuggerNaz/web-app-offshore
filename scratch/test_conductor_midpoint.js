...
async function run() {
    try {
        ...
        // Now evaluate conductor logic
        console.log("\nResolving Conductors:");
        ...
        conductors.forEach(c => {
            ...
            const cgf = components.find(other => {
                const otherCode = (other.code || "").toUpperCase();
                const otherMd = other.metadata || {};
                return (otherCode === "CF" || (other.q_id || "").toUpperCase().includes("CGF")) &&
                       (otherMd.s_node || "").toUpperCase().trim() === (md.s_node || "").toUpperCase().trim();
            });
            ...
            if (cgf) {
                ...
                if (sPos && fPos) {
                    ...
                    resolved = true;
                    console.log(`Conductor ${c.q_id} resolved via guide frame ${cgf.q_id} (connecting ${cgfMd.s_node} & ${cgfMd.f_node}):`);
                    console.log(`  Start: [${start.x.toFixed(3)}, ${start.y.toFixed(3)}, ${start.z.toFixed(3)}]`);
                    console.log(`  End:   [${end.x.toFixed(3)}, ${end.y.toFixed(3)}, ${end.z.toFixed(3)}]`);
                }
            }
            ...
            if (!resolved) {
                ...
                if (startNode) {
                    start.set(startNode.x, topElv, startNode.z);
                    end.set(startNode.x, botElv, startNode.z);
                    resolved = true;
                    console.log(`Conductor ${c.q_id} resolved via fallback to s_node ${md.s_node}:`);
                    console.log(`  Start: [${start.x.toFixed(3)}, ${start.y.toFixed(3)}, ${start.z.toFixed(3)}]`);
                    console.log(`  End:   [${end.x.toFixed(3)}, ${end.y.toFixed(3)}, ${end.z.toFixed(3)}]`);
                } else {
                    console.log(`Conductor ${c.q_id} failed to resolve!`);
                }
            }
        });
