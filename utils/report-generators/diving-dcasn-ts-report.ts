...
export const generateDivingDCASNTSReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Caisson Inspection Topside Diving", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        sortedCaissonQids.forEach((caissonQid, groupIdx) => {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Caisson Inspection Topside Diving  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`Caisson_Inspection_Topside_Diving_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
