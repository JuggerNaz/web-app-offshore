...
/**
 * ROV Caisson Guard Inspection Report (Portrait)
 * Columns: Item No. | QID | Elevation | Dive No. | Tape No. | CP | Findings
 *
 * Data is grouped by Caisson Guard (SG / CS_GUARD). Each SG group starts on a new page.
 * Associated components (like anodes, clamps attached to SG) are clubbed with their parent SG QID.
 */
export const generateROVCaissonGuardReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11);  d.setFont("helvetica", "bold");
            d.text("Caisson Guard Inspection Report (ROV)",                             margin + contentWidth / 2, margin + 16.5, { align: "center" });
        ...
        // ── Grouping Logic ──────────────────────────────────────────────────────
        const idToComp: Record<number, { q_id: string, name: string, parent_id: number | null, is_sg: boolean }> = {};
        
        const addCompToMap = (c: any) => {
            if (!c || !c.id) return;
            const qid = (c.q_id || "").toUpperCase();
            const typeCode = (c.code || c.metadata?.type || "").toUpperCase();
            const compName = c.comp_name || c.name || "Caisson Guard";

            // Group by Caisson Guard (SG / CS_GUARD)
            const isSG = qid.startsWith("SG") || qid.startsWith("CS_GUARD") || qid.startsWith("CS-GUARD") || (qid.includes("GUARD") && qid.includes("CS")) || typeCode === "SG" || typeCode === "CAISSONGUARD" || (c.code || "").toUpperCase() === "SG" || compName.toUpperCase().includes("CAISSON GUARD");
        ...
        // 4. Grouping Logic
        const sgGroups: Record<number, any[]> = {};
        records.forEach(r => {
            const comp = r.structure_components || r.component || {};
            const metadata = comp.metadata || {};
            ...
            // ONLY include if the resolved group belongs to a SG component type
            const parentInfo = idToComp[groupId];
            if (groupId && parentInfo?.is_sg) {
                if (!sgGroups[groupId]) sgGroups[groupId] = [];
                sgGroups[groupId].push(r);
            }
        });
        ...
        sortedParentIds.forEach((parentId, groupIdx) => {
            ...
            const groupRecords = sgGroups[parentId].sort((a, b) => {
            ...
            const parentName = parentComp?.name || "Caisson Guard";

            drawPageHeader(doc);
            const startY = drawContextRow(doc, margin + HEADER_H + 2, groupRecords);

            // SG Section Label (Sub-header)
            const subH = 6;
            const subY = startY;
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`ROV_Caisson_Guard_Inspection_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[ROV Caisson Guard Report] Error:", err);
        throw err;
    }
};
