...
export const generateROVRSEABCraterDetailReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Seabed Survey Crater Inspection Report (ROV)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        doc.save(`ROV_Seabed_Survey_Crater_Inspection_Report_${(config?.reportNoPrefix || headerData?.sowReportNo)}_${format(new Date(), 'yyyyMMdd')}.pdf`);
    } catch (e) {
