...
export const generateDivingCPCLBReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("CP Calibration Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        doc.save(`Diving_CP_Calibration_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
