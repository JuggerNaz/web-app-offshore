...
export const generateROVConductorGuardReport = async (
    ...
    try {
        ...
        sortedParentIds.forEach((parentId, groupIdx) => {
            ...
            autoTable(doc, {
                ...
                didDrawPage: (data) => {
                    ...
                    doc.text(
                        `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  ROV Conductor Guard Inspection Report  |  SOW: ${headerData.sowReportNo || "N/A"}`,
                        margin, pageHeight - 6
                    );
        ...
        doc.save(`ROV_Conductor_Guard_Inspection_Report_${headerData.sowReportNo || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
