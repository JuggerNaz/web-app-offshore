...
export function LevelDialog() {
    ...
    const { data: elvData, error: elvError, isLoading: elvIsLoading } = useSWR(`/api/platform/elevation/${pageId}`, fetcher)
                ...
                <div className="grid items-center gap-4">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                            <RowWrap>
                                <ColWrap>
                                    <FormFieldWrap label="Name" name="level_name" form={form} placeholder="" ftype="normal" />
                                    <FormFieldWrap label="From Elevation" name="elv_from" options={elvData.data.map((x:any) => {
                                        return { label: x.elv.toString(), value: x.elv.toString() }
                                    })} form={form} ftype="select" 
                                    />
                                    <FormFieldWrap label="To Elevation" name="elv_to" options={elvData.data.map((x:any) => {
                                        return { label: x.elv.toString(), value: x.elv.toString() }
                                    })} form={form} ftype="select" 
                                    />
