...
/**
 * Diving Close Visual Inspection (CVINS) Report
 */
export const generateDivingCVINSReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        const drawPageFooter = (d: jsPDF, pageNo: number) => {
            ...
            d.text(
                `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Diving CVINS Report  |  SOW: ${headerData.sowReportNo || "N/A"}`,
                margin, pageHeight - 6
            );
        ...
        if (records.length === 0) {
            ...
            doc.text("No records found for CVINS.", margin, margin + HEADER_H + 20);
        } else {
            let pageNum = 1;
            ...
            // Table Data
            // Details as follows, Item No. , QID, Elevation, Length, Width, Lighting Method, Findings.
            const tableBody = records.map((r, index) => {
                ...
                const length = d.length !== undefined && d.length !== null ? `${d.length} ${d.length_unit || 'mm'}` : "—";
                const width = d.width !== undefined && d.width !== null ? `${d.width} ${d.width_unit || 'mm'}` : "—";
                const lightingMethod = d.lighting_method || "—";
        ...
        doc.save(`Diving_CVINS_Report_${headerData.sowReportNo || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
