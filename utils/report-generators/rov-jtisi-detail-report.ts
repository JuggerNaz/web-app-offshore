...
/**
 * ROV J-Tube Inspection Report (Portrait)
 * Columns: Item No. | QID | Elevation | Dive No. | Tape No. | CP (mV) | Findings
 *
 * Data is grouped by J-Tube parent (RS). Each J-Tube group starts on a new page.
 * Associated components are clubbed under their parent J-Tube QID.
 * Inside each group, data is ordered by Elevation ascending.
 */
export const generateROVRRISIJTubeDetailReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        // ── Filter Records (Strict J-Tube Filter: RRISI only + Prefix J only) ──
        ...
        if (unassigned.length > 0) {
            if (jtubesMap.size === 1) Array.from(jtubesMap.values())[0].records.push(...unassigned);
            else jtubesMap.set(0, { jtubeComp: { q_id: 'Miscellaneous' }, records: unassigned });
        }

        const groups = Array.from(jtubesMap.values()).sort((a, b) => {
            const qA = a.jtubeComp?.q_id || '';
        ...
        doc.save(`ROV_JTube_Inspection_Report_${(config?.reportNoPrefix || headerData?.sowReportNo)}_${format(new Date(), 'yyyyMMdd')}.pdf`);
    } catch (e) {
        console.error("ROV J-Tube Detail Report Error", e);
        throw e;
    }
};
