...
export const generateDivingDCONDUWReport = async (
    ...
    console.log("[Diving Conductor UW Report] Starting generation", { recordsCount: records?.length, config });
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Conductor Inspection Underwater Diving", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        sortedConductorQids.forEach((conductorQid, groupIdx) => {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Conductor Inspection Underwater Diving  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        console.log("[Diving Conductor UW Report] Generation complete, returnBlob:", config?.returnBlob);
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`Conductor_Inspection_Underwater_Diving_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[Diving Conductor UW Report] Error:", err);
        throw err;
    }
};
