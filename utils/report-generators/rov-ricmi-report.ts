...
/**
 * ROV Inclinometer Reading Inspection Report (Portrait)
 * Columns: Item No. | Component QID | Elevation (m) | Dive No. | Angle Reading (deg) | Findings
 */
export const generateROVRICMIReport = async (
    records: any[],
    headerData: any,
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.text("Inclinometer Reading Report (ROV)",                     margin + contentWidth / 2, margin + 17, { align: "center" });
        ...
        const buildRow = (r: any, idx: number): string[] => {
            ...
            const elevation = r.elevation ?? d.elevation !== undefined ? `${r.elevation ?? d.elevation} m` : "—";
            ...
            const angleReading = d.angle_rdg !== "" && d.angle_rdg !== null && d.angle_rdg !== undefined
                ? `${d.angle_rdg} deg`
                : "—";

            const findingsParts: string[] = [];

            // Add additional inclinometer readings
            const additionals: any[] = Array.isArray(d.inc_reading_additional) ? d.inc_reading_additional : [];
            additionals.forEach((a: any) => {
                const val = a.reading ?? "";
                if (val !== "" && val !== null && val !== undefined) {
                    const loc = a.location ? ` @ ${a.location}` : "";
                    findingsParts.push(`Add. Angle${loc}: ${val} deg`);
                }
            });
        ...
        autoTable(doc, {
            ...
            didDrawPage: (data) => {
                ...
                doc.text(
                    `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  Inclinometer Reading Report (ROV)  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                    margin, pageHeight - 6
                );
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`ROV_RICMI_Inclinometer_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
