...
export const generateROVConductorGuardReport = async (
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.text("ROV Conductor Guard Inspection Report",                             margin + contentWidth / 2, margin + 17, { align: "center" });
        ...
        // ── Grouping Logic ──────────────────────────────────────────────────────
        const idToComp: Record<number, { q_id: string, name: string, parent_id: number | null, is_cu: boolean }> = {};
        ...
        // 4. Grouping Logic
        const cuGroups: Record<number, any[]> = {};
        records.forEach(r => {
            const comp = r.structure_components || r.component || {};
            const metadata = comp.metadata || {};
            ...
            // ONLY include if the resolved group belongs to a CU component type
            const parentInfo = idToComp[groupId];
            if (groupId && parentInfo?.is_cu) {
                if (!cuGroups[groupId]) cuGroups[groupId] = [];
                cuGroups[groupId].push(r);
            }
        });
        ...
        sortedParentIds.forEach((parentId, groupIdx) => {
            ...
            const parentName = parentComp?.name || "Conductor Guard";

            drawPageHeader(doc);
            const startY = drawContextRow(doc, margin + HEADER_H + 2, groupRecords);

            // CU Section Label (Sub-header)
            const subH = 6;
            const subY = startY;
            ...
            autoTable(doc, {
                ...
                didDrawPage: (data) => {
                    ...
                    doc.text(
                        `${companySettings.company_name || "NasQuest Resources Sdn Bhd"}  |  ROV Conductor Guard Inspection Report  |  SOW: ${(config?.reportNoPrefix || headerData?.sowReportNo) || "N/A"}`,
                        margin, pageHeight - 6
                    );
        ...
        applyWatermarkAndSignaturesGlobal(doc, config);
        doc.save(`ROV_Conductor_Guard_Inspection_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[ROV Conductor Guard Report] Error:", err);
        throw err;
    }
};
