...
function KPICard({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border p-5 flex flex-col gap-3 transition-all hover:scale-[1.02] hover:shadow-lg ${color}`}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${accent}`}>
            {(React.createElement as any)(Icon, { className: "w-4 h-4" })}
          </div>
          <span className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 dark:text-slate-400">
            {label}
          </span>
        </div>
        {children}
      ...
      {sub && (
        <div className="text-[10px] font-medium text-slate-500 dark:text-slate-500 leading-tight">
          {sub}
        </div>
      )}
    </div>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <KPICard
        icon={Gauge}
        label="CP Readings"
        value={data.cpAvg !== null ? `${data.cpAvg}mV` : "N/A"}
      ...
      <KPICard
        icon={Zap}
        label="Anode Health"
        value={data.anodeAvgDepletion !== null ? `${data.anodeAvgDepletion}%` : "N/A"}
      ...
      <KPICard
        icon={Eye}
        label="FMD Status"
        value={`${data.fmdFlooded}/${data.fmdTotal}`}
