...
/**
 * Conductor Inspection Underwater Report (Diving)
...
export const generateDivingDCONDUWReport = async (
    ...
    console.log("[generateDivingDCONDUWReport] Starting generation", { recordsCount: records?.length, config });
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Conductor Inspection Underwater Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        doc.save(`Diving_Conductor_Inspection_Underwater_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[generateDivingDCONDUWReport] Error:", err);
        throw err;
    }
};
