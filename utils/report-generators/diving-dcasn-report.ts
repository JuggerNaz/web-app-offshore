...
/**
 * Caisson Inspection Report (Diving)
...
export const generateDivingDCASNReport = async (
    ...
    console.log("[generateDivingDCASNReport] Starting generation", { recordsCount: records?.length, config });
    ...
    try {
        ...
        const drawPageHeader = (d: jsPDF) => {
            ...
            d.setFontSize(11); d.setFont("helvetica", "bold");
            d.text("Caisson Inspection Report (Diving)", margin + (contentWidth / 2), margin + 16.5, { align: "center" });
        ...
        doc.save(`Diving_Caisson_Inspection_Report_${(config?.reportNoPrefix || headerData?.sowReportNo) || "NOSO"}_${format(new Date(), "yyyyMMdd")}.pdf`);
    } catch (err) {
        console.error("[generateDivingDCASNReport] Error:", err);
        throw err;
    }
};
