...
async function run() {
    try {
        ...
        console.log("Interpolating child welds (WITH RELATIVE DISTANCE CODE FIX):");
        for (let iter = 0; iter < 3; iter++) {
            childWeldGroups.forEach((children, parentId) => {
                ...
                // Find startNodeWeld and endNodeWeld to compute relative distance bounds
                ...
                children.forEach((c, idx) => {
                    ...
                    if (["WN N12", "WN N18", "WN N13", "WN N19"].includes(c.metadata?.description)) {
                        ...
                        console.log(`  Parent ${parentComp.id} (${parentMd.description}): s_node=${parentMd.s_node} at [${pStart.x.toFixed(3)}, ${pStart.y.toFixed(3)}, ${pStart.z.toFixed(3)}], f_node=${parentMd.f_node} at [${pEnd.x.toFixed(3)}, ${pEnd.y.toFixed(3)}, ${pEnd.z.toFixed(3)}]`);
                        console.log(`  dist=${c.metadata.dist}, startDist=${startDist}, endDist=${endDist}, t=${t.toFixed(4)}`);
